...
//        stk::math::max(aOne * sdr.get(meshIdx, 0), sijMag * fTwo);
	...
	u_par_mag += u_par_vec[i] * u_par_vec[i];
      ...
      const DblType Re_buri = u_par_mag*minD.get(meshIdx, 0)*density.get(meshIdx,0)/visc.get(meshIdx, 0); 
      //const DblType Gamma = sThres;
      const DblType sThres = -5e-4;
      DblType lamda0L_thres = sThres*pow(Re_buri,0.75); // turbulent criterion
	...
	tvisc.get(meshIdx, 0) =
	  0.275 * density.get(meshIdx, 0) * tke.get(meshIdx, 0) /
	  (sijMag * fTwo);
